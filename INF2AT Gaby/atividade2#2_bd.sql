...
select pk_cod_atendimento,
valor,
valor+300
from atendimento;

select pk_cod_atendimento,atendimentoatendimentoatendimento
valor,
10*(valor + valor)*0.17
from atendimento;
...
select pk_cod_pac codigo,
nome as paciente
from paciente;
select nome "paciente preferencial"
from paciente;

select concat(pk_cod_pac, nome) as "paciente";
from paciente;
select concat(pk_cod_pac, ' ', nome) as "paciente"
from paciente;

select concat(nome, ' ', 'nasceu em', ' ', dt_nasc)
as "nascimento do paciente"
from  paciente;
...
-- 3
select concat(pk_cod_plano," ", nome_plano)
"plano de saúde"
from plano_saude;
...
select pk_cod_atendimento, 
		...
		valor,
        desconto,
        valor-desconto as "Valor a pagar"
from atendimento;
